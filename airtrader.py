            ...
            if n_sell_order < 0:
                ...
                for ask, ask_amount in state.order_depths[product].sell_orders.items():
                    if ask <= buy_threshold[product]:
                        ...
                        found_best = True
                        sell_order = Order(product, ask, n_sell_order)
                        break
                if not found_best:
            ...
            if n_buy_order > 0:
                ...
                for bid, bid_amount in state.order_depths[product].buy_orders.items():
                    if bid >= sell_threshold[product]:
                        ...
                        found_best = True
                        buy_order = Order(product, bid, n_buy_order)
                        break
                if not found_best:
...
def get_product_edge(state, product, buy_sell):
    ...
    if product == "AMETHYSTS":
        ...
        if buy_sell == "buy":
            return (np.std(price_history) / np.mean(price_history))/1.25
        else:
            return (np.std(price_history) / np.mean(price_history))/3.5
